...
Model::Model() :
    ...
    _turb_coeff(0.5), _texture_scale(10)
{
...
void Model::draw_mesh(glm::mat4 model, glm::mat4 user, glm::mat4 view, glm::mat4 projection)
{
    ...
    if (_shading_mode == SHADING_MODE_COLOR) {
        ...
        if (_shading_mode == SHADING_MODE_PHONG) {
            glUniform3f(glGetUniformLocation(program, "lowerLeft"),
                _lower_left[0], _lower_left[1], _lower_left[2]);
            glUniform3f(glGetUniformLocation(program, "upperRight"),
                _upper_right[0], _upper_right[1], _upper_right[2]);
            glUniform1f(glGetUniformLocation(program, "turbulenceCoeff"), _turb_coeff);
            glUniform1f(glGetUniformLocation(program, "textureScale"), _texture_scale);
        }
...
void Model::increase_texture_scale()
{
    _texture_scale = _texture_scale + 1.f;
}

void Model::decrease_texture_scale()
{
    _texture_scale = std::max(_texture_scale - 1.f, 1.f);
}

void Model::increase_turb_coeff()
{
    _turb_coeff = std::min(_turb_coeff + 0.1f, 1.f);
}

void Model::decrease_turb_coeff()
{
    _turb_coeff = std::max(_turb_coeff - 0.1f, 0.f);
}
